...
def main():
    ...
    Will combine all available TritonFeatures.tsv files in the results directory.
    """
    ...
    # Find all files in results_dir that end with "_TritonFeatures.tsv"
    inputs = glob.glob(args.results_dir + "/*_TritonFeatures.tsv")

    df_list = [pd.read_table(filename, sep='\t', index_col=0, header=0) for filename in inputs]
    ...
    if args.output_format == 'wide':  # only for readability
        ...
        if os.path.exists(args.results_dir + '/TritonCompositeFM.tsv'):
            old_df = pd.read_table(args.results_dir + '/TritonCompositeFM.tsv', sep='\t', index_col=False, header=0)
