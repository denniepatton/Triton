...
grouping or palette options set. LuCaP_136CR is ARPC and shows a clear dip in depth and increase in diversity at the center, unlike LuCaP_145-2 (NEPC) and NPH001 (healthy donor):
<img src="misc/ATAC_AD-Exclusive_filtered_sig-Profiles_RSD.png">

And here is another example using sample groups, with categories and a palette specified, looking at an individual region - the AR promoter centered at the transcription start site (TSS).
In this example a dip in ARPC lines' coverage is seen just upstream of the TSS, which is not seen in NEPC and healthy lines, indicative of active translation.
...
Here is an example of this plotting mode for 1,000 ASCL1 binding sites (TFBSs) run in composite mode. As ASCL1 is active in NEPC lines, but not in ARPC or healthy lines,
...
"signal" mode plots only the phased-nucleosome signal, and is useful for visualizing nucleosome depletion.  
